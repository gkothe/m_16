...
public class Relatorios {
	...
	public static JRMapCollectionDataSource relPedidosDataSource(int coddistr, Connection conn, Map hmParams, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			hmParams.put("modo_pay", id_modopagamento.equalsIgnoreCase("") ? "Todos" : Utilitario.returnDescPagamento(conn, Integer.parseInt(id_modopagamento)));
			if (!cod_bairro.equalsIgnoreCase(""))
			...
			sql.append("       pedido_item.val_unit, modo_pagamento.desc_modo, ");
			sql.append("       qtd_prod, ");
			...
			sql.append(" 	   left join modo_pagamento  ");
			sql.append(" 			  on modo_pagamento.id_modo_pagamento  =   pedido.id_modo_pagamento ");
			...
			if (!id_modopagamento.equalsIgnoreCase("")) {
			...
			if (!id_modopagamento.equalsIgnoreCase("")) {
				st.setInt(contparam, Integer.parseInt(id_modopagamento));
				contparam++;
			}
			...
			while (rs.next()) {
				...
				hmFat.put("desc_modopagamento", rs.getString("desc_modo"));
	...
	public static void rodaRel(String nome, JRMapCollectionDataSource datasource, Map hmParams, HttpServletRequest request, HttpServletResponse response, Sys_parametros sys) {
		...
		try {
			...
			JasperReport objRelJasper_ordemprod = null;
			 objRelJasper_ordemprod = JasperCompileManager.compileReport("D:/phonegap_projects/m_16/newreqs/src/main/webapp/rels/" + nome + ".jrxml");
			 String arq = "" + new File("D:/phonegap_projects/m_16/newreqs/src/main/webapp/rels/" + nome + ".pdf");
